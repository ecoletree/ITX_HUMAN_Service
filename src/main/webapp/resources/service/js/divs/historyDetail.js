...
(function(et, ctrl) {
	...
	ctrl.init = function(initData) {
		...
		$(".btnDial").on("click", self.btnDial_clickEventListener);
	...
	ctrl.sendData = function(data) {
		...
		if (!!self.detailData) {
			...
			$(".btnDial").data("tel",self.detailData.hand_tel);
			...
			var memo1 = self.detailData.cr_mm_add === null ? "" : (self.detailData.cr_mm_add).replace(/<br\s*\/?>/mg,"\n");
			$("#"+self.name+"_taCr_mm").val(memo);
			$("#"+self.name+"_taCr_mm_add").val(memo1);
...
//			$("#"+self.name+"_dfCalss_4").attr("disabled", "disabled");
	...
	ctrl.btnDial_clickEventListener = _.debounce(function() {
		var self = et.historyDetail;
		var telNumber = $(".btnDial").data("tel");
		if (telNumber !== "") {
