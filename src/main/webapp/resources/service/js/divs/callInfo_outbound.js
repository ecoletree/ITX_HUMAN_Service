...
(function(et, ctrl) {
	...
	ctrl.init = function(initData) {
		...
		// set Event
		$("#"+self.name+"_btnOpenCustInfoModal").on("click", self.btnOpenCustInfoModal_clickEventListener);
...
//			self.afterCallServiceStartTime = $.format.date(new Date(), 'yyyy-MM-dd HH:mm:ss'); 
	...
	ctrl.makeCodeList = function(target, list) {
		...
		if (!!list) {
			for (var i = 0; i < list.length; i++) {
				...
				li.data("num", num);
				li.data("value1", list[i]["value1"]);
				li.data("value2", list[i]["value2"]);
				$(target).append(li);
	...
	ctrl.makeSaveParam = function(type) {
		...
		if (type === "call" || type === "both") {
			...
			self.callData.o_state_cd = $("#"+self.name+"_ulCrCd4 > li").filter(".sel").data("value1");
			self.callData.o_state_scd = $("#"+self.name+"_ulCrCd4 > li").filter(".sel").data("value2");
	...
	ctrl.btnOpenCustInfoModal_clickEventListener = function() {
		$("#home_divUserInfoDetail").show();
	};
