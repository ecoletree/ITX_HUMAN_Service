...
(function(et, ctrl) {
	...
	ctrl.api_url = "https://sp2.hsitx-ccc.com"; // 효성 리얼서버
...
//	ctrl.api_url = "https://hsitx-app.ga"; // 이콜트리 테스트서버
	...
	ctrl.init = function(initData, api_url) {
		...
		$("#"+self.name+"_divUserInfoDetail").draggable({cancel:'.popupBody'});
		$("#"+self.name+"_divOutbound").draggable({cancel:'.popupBody'});
		...
		$("#"+self.name+"_imgOutbound").click(self.shortCutClickHandler);
	...
	ctrl.setOutboundData = function(oLayout, oData) {
		var self = et.home;
		
		$("#home_divOutboundDetailInfo").empty();
		$("#callInfo_iptCust_id").val(oData.cust_id);		
		
		_.forEach(oLayout, function(item) {
			var sValue = oData[item.layout_cd] === null ? "" : oData[item.layout_cd];
			
			if (item.is_prime === "Y") {
				if (item.layout_type === "001") $("#callInfo_iptCust_nm").val(sValue);
				else if (item.layout_type === "002") $("#callInfo_iptHand_tel").val(sValue);
			
			} else {
				$("#home_divOutboundDetailInfo").append('<div class="marginB30"><h6>'+item.header_name+'</h6><p>'+sValue+'</p></div>');
			}
		});
			...
			try {
				window.bpspat.api.getState(function (data) {
					...
					if (data.data.agent_id !== $("#home_spTmrId").text()){
						...
						if (data.data.status === BP_STATE.BUSY) {
							self.customer = {};
							self.isFirst = true;
							self.callIng = true;
	...
	ctrl.bpspatHandler = function (data) {
		...
		console.log(data);
		try {
			if (data.command === BP_COMMAND.STATE_CHANGED ) {
				if (data.data.status === BP_STATE.READY) {  // 상태가 ready 이면 그때 고객 정보를 초기화
					self.customer = {};
				} else if (data.data.status === BP_STATE.AFTER_CALL_WORK) { // 후처리 상태에서는 화면 컨트롤
	 ...
	 * 아웃바운드 프리뷰 모달 띄우기
	 */
	ctrl.openOutboundModal = _.debounce(function () {
		et.outbound.showOutboundModal();
	},500);
